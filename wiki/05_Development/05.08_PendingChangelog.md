...
- **Screenplay Print Format**
    ...
    - Fixed hardcoded element spacing (removed `mb-3` class) - now recipe's lineSpaceBefore/lineSpaceAfter have full control
    ...
    - Improved page break handling for dialogue:
        ...
        - Dialogue split across pages now shows "(MORE)" at bottom of page
        - Character name repeated with " (CONT'D)" appended at top of next page
...
- **Simplified Margin API**: Replaced `leftMargin` and `rightMargin` with a single `margin` property
