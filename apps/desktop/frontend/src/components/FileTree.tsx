...
function FileTree() {
  ...
  const handleOpenDirectory = async () => {
    try {
      ...
      toast.error(`Failed to open directory: ${formatError(error)}`);
    }
  };
  ...
  const loadDirectoryContents = async (dirPath: string) => {
    try {
      ...
      toast.error(`Failed to read directory: ${formatError(error)}`);
      setFiles([]);
    }
  };
        ...
        {!rootDirectory ? (
          <div className="p-4 text-center text-base-content/50 text-sm">
            ...
            <p className="mt-2 text-xs">
              Click "Open Directory" to browse files
            </p>
          </div>
        ) : files.length === 0 ? (
          <div className="p-4 text-center text-base-content/50 text-sm">
          ...
          <ul className="menu p-2 text-sm">
            {files
              ...
              .map((file) => {
                const isNonCellFile =
                  !file.isDirectory && !isCellFile(file.name);
