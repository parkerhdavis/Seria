 ...
 * For new code, consider using the unified pipeline from dataTransformationPipeline.ts
 * which provides composable parse -> transform -> serialize operations.
 */

import Papa from "papaparse";
import { CellData } from "@/types/cellData";
import type { WorkerResponse } from "./cellParser.worker";
import { logger } from "@/utils/logger";

// Re-export pipeline utilities for consumers that want the full pipeline
export {
  parse as pipelineParse,
  executePipeline,
  applyTransforms,
 ...
 */
export function validateCell(cellData: CellData): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
