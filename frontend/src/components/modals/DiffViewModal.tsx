...
export default function DiffViewModal({ isOpen, onClose }: DiffViewModalProps) {
    ...
    const allChanges = useMemo(() => {
        ...
        const changes: Array<{ type: "added" | "deleted" | "modified"; row: number; side: "left" | "right" }> = [];
    ...
    const performDiff = async (compareWith: string) => {
        ...
        try {
            ...
            const compareContent = await invoke<string>("open_cell_file", { path: compareWith });
    ...
    const goToChange = (index: number) => {
        ...
        const rowEl = document.querySelector(`[data-diff-row="${change.side}-${change.row}"]`);
        if (rowEl) {
    ...
    const maxRows = Math.max(diffResult?.oldRowCount ?? 0, diffResult?.newRowCount ?? 0);
        ...
        <div className="modal modal-open" onKeyDown={handleKeyDown}>
            <div className="modal-box max-w-[95vw] w-[95vw] max-h-[90vh] h-[90vh] flex flex-col">
                ...
                <div className="flex items-center gap-4 mb-4">
                    <button className="btn btn-sm btn-primary" onClick={handlePickFile} disabled={isLoading}>
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-mono">
                                    {allChanges.length > 0 ? `${currentChangeIndex + 1} / ${allChanges.length}` : "0 changes"}
                                </span>
                                <button
                                    className="btn btn-sm btn-ghost"
                                    onClick={nextChange}
                    ...
                    <div className="flex flex-col items-center justify-center flex-1 text-base-content/50">
                        ...
                        <p className="text-center mb-4">
                            ...
                            <span className="font-semibold">
                                {fileInfo?.name || currentFile?.split("/").pop() || "the current file"}
                            </span>
                        </p>
                        <p className="text-xs text-base-content/40">
                            The selected file will be shown on the left (old), current file on the right (new)
                        </p>
                    </div>
                )}
                    ...
                    <div className="flex flex-1 gap-1 overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden border border-base-300 rounded-lg">
                            <div className="px-3 py-2 bg-base-200 border-b border-base-300 text-sm font-semibold flex items-center gap-2">
                                ...
                                {compareFilePath?.split("/").pop() || compareFilePath?.split("\\").pop() || "Previous"}
                                <span className="text-base-content/50 font-normal">
                                    ({diffResult.oldRowCount} rows, {diffResult.oldHeaders.length} cols)
                                </span>
                            </div>
                            <div
                                ref={leftPanelRef}
                                                        ...
                                                        <td
                                                            ...
                                                            className={
                                                                isDeleted
                                                                    ...
                                                                    : diffResult.columnChanges.deleted.includes(diffResult.oldHeaders[colIdx])
                                                                        ? "bg-error/5"
                                                                        : ""
                                                            }
                                                        >
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden border border-base-300 rounded-lg">
                            <div className="px-3 py-2 bg-base-200 border-b border-base-300 text-sm font-semibold flex items-center gap-2">
                                ...
                                <span className="text-base-content/50 font-normal">
                                    ({diffResult.newRowCount} rows, {diffResult.newHeaders.length} cols)
                                </span>
                            </div>
                            <div
                                ref={rightPanelRef}
                                                            ...
                                                            <td
                                                                ...
                                                                className={
                                                                    isAdded
                                                                        ...
                                                                        : mod
                                                                            ? "bg-warning/20"
                                                                            : diffResult.columnChanges.added.includes(diffResult.newHeaders[colIdx])
                                                                                ? "bg-success/5"
                                                                                : ""
                                                                }
                                                                title={mod ? `Was: "${mod.oldValue}"` : undefined}
                                                            >
                                                                <span className="max-w-[200px] truncate block">
                                                                    {mod ? (
                                                                        <span className="relative">
                                                                            <span className="bg-warning/30 rounded px-0.5">{cell}</span>
                                                                        </span>
                                                                    ) : (
                                                                        cell
                                                                    )}
                ...
                <div className="modal-action">
                    <div className="flex items-center gap-2 text-xs text-base-content/40 mr-auto">
                        <span className="inline-block w-3 h-3 bg-success/20 border border-success/30 rounded"></span> Added
                        <span className="inline-block w-3 h-3 bg-error/20 border border-error/30 rounded ml-2"></span> Deleted
                        <span className="inline-block w-3 h-3 bg-warning/20 border border-warning/30 rounded ml-2"></span> Modified
                    </div>
