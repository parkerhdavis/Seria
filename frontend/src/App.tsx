...
function App() {
  ...
  const loadCellsProgressive = useCellStore(
    (state) => state.loadCellsProgressive,
  );
  ...
  const setRightDrawerSize = useDrawerStore(
    (state) => state.setRightDrawerSize,
  );
  const setBottomDrawerSize = useDrawerStore(
    (state) => state.setBottomDrawerSize,
  );
  ...
  const hoverHighlightMode = useSettingsStore(
    (state) => state.hoverHighlightMode,
  );
  ...
  useEffect(() => {
    const handleKeyDown = async (e: KeyboardEvent) => {
        ...
        try {
          ...
          if (isErrorWithMessage(error, "TEMP_FILE_NEEDS_LOCATION")) {
            ...
            try {
              ...
              toast.error(`Save failed: ${formatError(saveError)}`);
              setIsFilePickerOpen(false);
            }
          } else {
      ...
      // Ctrl+1 through Ctrl+9 - Quick switch to layout presets
        ...
        const layouts = useWorkspaceStore
          .getState()
          .layouts.sort((a, b) => b.lastUsed - a.lastUsed);

        if (layouts[layoutIndex]) {
          const layout = useWorkspaceStore
            .getState()
            .loadLayout(layouts[layoutIndex].id);
          if (layout) {
            applyWorkspaceLayout(layout);
            useWorkspaceStore
              .getState()
              .updateLayoutUsage(layouts[layoutIndex].id);
          }
        }
      }
      // Ctrl+Z - Undo
  ...
  return (
    <DragProvider>
      ...
      >
        ...
        <GoToModal isOpen={isGoToOpen} onClose={() => setIsGoToOpen(false)} />
